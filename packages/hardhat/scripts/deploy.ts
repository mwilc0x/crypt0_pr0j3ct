...
function deploy() {
    ...
    if (network == undefined) {
        ...
        exec(
            `hardhat --network ${network} deploy --gasprice 50000000000 --export-all ${nftMarketClientDeploySrc}/hardhat_contracts.json`, 
    ...
    function copyGenerated() {
        ...
        copies.forEach(copy => {
            ...
            shell(`mkdir -p ${copy}`);
            shell(`cp -r ${nftMarketClientDeploySrc}/* ${copy}`);
        });
    }
}
